(() => {
  const columns = [
    ["original", "原始渲染图"],
    ["realtime_first", "首轮 LoRA"],
    ["realtime_second", "第二轮 LoRA"],
    ["beauty_first", "首轮 LoRA"],
    ["beauty_second", "第二轮 LoRA"],
  ];
  ...
  function render() {
    ...
    app.innerHTML = `<div class="app">
      <header class="topbar"><div class="brand"><div><h1>首轮 vs 第二轮 LoRA 效果横向对比</h1><p>不再对比线上效果，聚焦首轮与第二轮 LoRA 在整体表现与 Badcase 专项中的差异</p></div></div><div class="summary"><strong>${scenes.length}</strong>/ ${total} 组</div></header>
  ...
  function tableHead() {
    return `<div class="th th-index compare-span">序号</div><div class="th compare-span">原始渲染图</div><div class="th compare-group group-realtime">实时增强</div><div class="th compare-group group-beauty">AI美化</div><div class="th compare-sub round-first">首轮 LoRA</div><div class="th compare-sub round-second">第二轮 LoRA</div><div class="th compare-sub round-first workflow-start">首轮 LoRA</div><div class="th compare-sub round-second">第二轮 LoRA</div>`;
  }

  function fullLabel(key) {
    return ({ original: "原始渲染图", realtime_first: "实时增强 · 首轮 LoRA", realtime_second: "实时增强 · 第二轮 LoRA", beauty_first: "AI美化 · 首轮 LoRA", beauty_second: "AI美化 · 第二轮 LoRA" })[key];
  }
  ...
  function row(scene) {
    return `<div class="td index-cell"><span>${String(scene.number).padStart(2, "0")}</span></div>${columns.map(([key]) => {
      ...
      const roundCompare = key === "realtime_second" ? `<button data-compare="${scene.id}" data-left="realtime_first" data-right="realtime_second" data-left-label="实时增强 · 首轮 LoRA" data-right-label="实时增强 · 第二轮 LoRA">轮次对比</button>` : key === "beauty_second" ? `<button data-compare="${scene.id}" data-left="beauty_first" data-right="beauty_second" data-left-label="AI美化 · 首轮 LoRA" data-right-label="AI美化 · 第二轮 LoRA">轮次对比</button>` : "";
      const buttons = original ? "" : `<div class="thumb-actions"><button data-compare="${scene.id}" data-left="original" data-right="${key}" data-left-label="原始渲染图" data-right-label="${label}">↔ 原图对比</button>${roundCompare}</div>`;
      return `<div class="td ${key === "beauty_first" ? "workflow-start" : ""}"><div class="thumb"><img src="${scene.thumbs?.[key] || scene.images[key]}" alt="${esc(scene.categoryName + " " + label)}" loading="lazy" decoding="async" data-single="${scene.id}" data-key="${key}" title="点击查看大图"/>${buttons}</div></div>`;
    }).join("")}`;
  }
